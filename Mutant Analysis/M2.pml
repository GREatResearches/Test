int battery = 3;
bool hasPwSrc = false, image = false, video = true, mobileGuide=true, showDocs=true, text=true;
...
active proctype Controller() {
	do
		:: buss?contextChanged -> 
			run AR02(); run AR03();run AR04();run AR05();
			numAnswer = 0;
			do 
				:: (numAnswer != 4) -> buss?done; numAnswer = numAnswer + 1;
				:: else -> break
			od
			buss!adapted	
	od	 
...
proctype AR02() {
	if
		:: (battery == 1 && hasPwSrc == true) -> buss!imageOn;buss!videoOff;buss!done
		:: else -> buss!done
	fi
}
...
proctype AR03() {
	if
		:: (battery == 2 && hasPwSrc == false) -> buss!imageOn;buss!videoOn;buss!done
		:: else -> buss!done
	fi
}
...
proctype AR04() {
	if
		:: (battery == 2 && hasPwSrc == true) ->  buss!imageOff;buss!videoOn;buss!done
		:: else -> buss!done
	fi
}
...
proctype AR05() {
	if
		:: (battery == 3) -> buss!imageOff;buss!videoOn;buss!done
		:: else -> buss!done
	fi
}
...
ltl pro33 {[] ((battery == 2 && hasPwSrc == false) -> <>(battery == 2 && hasPwSrc == false && image == true && video == true))}
ltl pro34 {[] ((battery == 2 && hasPwSrc == true) -> <>(battery == 2 && hasPwSrc == true && image == false && video == true))}
ltl pro35 {[] ((battery == 3) -> <>(battery == 3 && image == false && video == true))}
ltl pro41 {<> image}
ltl pro42 {<> video}
ltl pro51 {<> !image}
ltl pro52 {<> !video}
