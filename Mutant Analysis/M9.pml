...
mtype = {imageOn, imageOff, showDocsOff, showDocsOn, videoOn, videoOff,contextChanged,done,adapted}
int numAnswer = 0;
...
active proctype showDocsActuator() {
	do
		:: atomic{ buss?showDocsOn -> showDocs = true }
		:: atomic{ buss?showDocsOff -> showDocs = false }
	od	
}
...
proctype AR01() {
	if
		:: (battery == 1 && hasPwSrc == false) -> buss!showDocsOff;buss!videoOff;buss!done
		:: else -> buss!done
	fi
}
...
ltl pro25 {<> (battery == 3)}
ltl pro31 {[] ((battery == 1 && hasPwSrc == false) -> <>(battery == 1 && hasPwSrc == false && showDocs == false && video == false))}
