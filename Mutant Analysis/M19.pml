...
mtype = {imageOn, imageOff, textOff, textOn, videoOn, videoOff,contextChanged,done,adapted}
int numAnswer = 0;
...
active proctype textActuator() {
	do
		:: atomic{ buss?textOn -> text = true }
		:: atomic{ buss?textOff -> text = false }
	od	
}
...
proctype AR01() {
	if
		:: (battery == 1 && hasPwSrc == false) -> buss!textOff;buss!videoOff;buss!done
		:: else -> buss!done
	fi
}
...
ltl pro25 {<> (battery == 3)}
ltl pro31 {[] ((battery == 1 && hasPwSrc == false) -> <>(battery == 1 && hasPwSrc == false && text == false && video == false))}
